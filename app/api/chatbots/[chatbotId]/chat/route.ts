...
export async function POST(
  ...
  try {
    ...
    if (data.filename !== "") {
      const file = new File([data.file as BinaryLike], data.filename, {
        type: (data.file as Blob).type,
      });

      if ((data.file as Blob)?.size > 0) {
    ...
    if (openAiFile) {
      messageParams.file_ids = [openAiFile.id];
    }

    const createdMessage = await openai.beta.threads.messages.create(
      threadId!,
      messageParams as OpenAI.Beta.Threads.Messages.MessageCreateParams
    );
      ...
      async ({ sendMessage, forwardStream, sendDataMessage }) => {
        try {
                ...
                content: [
                  {
                    ...
                    text: {
                      value:
                        "You have reached your monthly message limit. Upgrade your plan to continue using your chatbot.",
                    },
                  },
                ],
              });
          ...
          if (runResult == undefined) {
            console.log(
              `Error running assistant ${chatbot.openaiId} on thread ${threadId}`
            );

            // set the error if last_error is not null
            let errorMessage = "Unknown error";
            const currentEvent = runStream.currentEvent();
            if (currentEvent && "last_error" in currentEvent.data) {
              errorMessage =
                (currentEvent.data as any).last_error?.message || errorMessage;
            }
            ...
            sendMessage({
              ...
              content: [
                {
                  type: "text",
                  text: { value: chatbot.chatbotErrorMessage },
                },
              ],
            });
            return;
