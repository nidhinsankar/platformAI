  ...
  try {
    ...
    if (!openAIConfig?.globalAPIKey) {
      return new Response("Missing OpenAI API key", {
        status: 400,
        statusText: "Missing OpenAI API key",
      });
    }
    ...
    if (content.toString().trim().length === 0) {
      console.error(
        "Failed to crawl URL contains only spaces:",
        crawler.crawlUrl + " - No content found"
      );
      return new Response(null, { status: 500 });
    }
    ...
    const date = new Date();
    const fileName =
      crawler.name.toLowerCase().replace(/\s/g, "-") +
      "-" +
      date.toISOString() +
      ".json";
