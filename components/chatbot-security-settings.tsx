...
import {
  advancedSettingsSecurityFormSchema,
  advancedSettingsSecuritySchema,
} from "@/lib/validations/advancedSettings";
...
export function ChatbotAdvancedSecuritySettingsForm({
  chatbot,
  ...props
}: ChatbotFormProps) {
  ...
  const [bannedIPsActiveTagIndex, setBannedIpsActiveTagIndex] = useState<
    number | null
  >(null);

  const [allowedIPs, setAllowedIPs] = useState<Tag[]>([]);
  const [allowedIPsActiveTagIndex, setAllowedIpsActiveTagIndex] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    setAllowedIPs(
      chatbot.allowedIpRanges.map((ip, index) => ({
        id: index.toString(),
        text: ip,
        label: ip,
        value: ip,
    ...
    setBannedIPs(
      chatbot.bannedIps.map((ip, index) => ({
        id: index.toString(),
        text: ip,
        label: ip,
        value: ip,
  ...
  async function onSubmit(data: FormData) {
    ...
    const response = await fetch(
      `/api/chatbots/${chatbot.id}/config/security`,
      {
        method: "PATCH",
        headers: {
    ...
    if (!response?.ok) {
      ...
      return toast({
        title: "Something went wrong.",
        description:
          "Your advanced settings are not updated. Please try again.",
        variant: "destructive",
      });
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    The feature allows you to ban IPs and IP ranges from
                    accessing the chatbot. Press ENTER after entering each IP or
                    IP range to add many.
                  </FormDescription>
                  <FormControl>
                    <TagInput
                      id="bannedIps"
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      This feature allows everyone to access the chatbot. You
                      must allow IPs to access the chatbot.
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    This feature allows you to specify the IPs and IP ranges
                    that can access the chatbot. Press ENTER after entering each
                    IP or IP range to add many.
                  </FormDescription>
                  <FormControl>
                    <TagInput
                      id="allowedIpRanges"
