...
export function ChatbotBrandingProSettingsForm({
  chatbot,
  className,
  ...props
}: ChatbotFormProps) {
  ...
  async function onSubmit(data: FormData) {
    ...
    const response = await fetch(
      `/api/chatbots/${chatbot.id}/config/branding`,
      {
        method: "PATCH",
        headers: {
                ...
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    <FormLabel className="text-base">
                      PlatformAI Branding Label
                    </FormLabel>
                    <FormDescription>
                      Remove &quot;Powered by PlatformAI&quot; from the chatbot.
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
