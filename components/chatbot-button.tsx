...
export default function ChatbotButton({
  ...
  useEffect(() => {
    ...
    window.parent.postMessage(
      { type: "checkScrollbar", hasScrollbar: hasScrollbar },
      "*"
    );
  }, []);
  ...
  return (
    ...
    >
      <button
        className={`user-select-none flex items-center justify-center absolute top-0 bottom-0 w-full transition-transform duration-300 ${
          animate ? "scale-125" : ""
        }`}
        onClick={toggleChatVisibility}
      >
        {!isChatVisible && (
          <Icons.message height={24} width={24} style={{ color: textColor }} />
        )}
